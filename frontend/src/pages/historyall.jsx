...
import { getHistoryAllApi } from '../util/api';
...
const HistoryAll = () => {
    ...
    const [selectedRow, setSelectedRow] = useState(null); // lưu dòng được chọn
    const [isModalVisible, setIsModalVisible] = useState(false);
    useEffect(() => {
        const fetchDetail = async () => {
            const res = await getHistoryAllApi();
            if (res.redirect) {
    ...
    return (
        <div className="p-4">
            <h2 className="text-xl font-semibold mb-4">Lịch sử đặt phòng</h2>
            <Table
                columns={columns}
                dataSource={historys}
                rowKey="TicketIDNum"
